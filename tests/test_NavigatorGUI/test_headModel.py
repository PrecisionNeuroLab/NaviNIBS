...
@pytest.mark.asyncio
@pytest.mark.order(after='test_setHeadModel')
@pytest.mark.parametrize('modelLabel,coordSysKeys', (
        ('Charm', ('FSAvg_SimNIBS_SBM',)),
        ('CharmFS', ('FSAvg_SimNIBS_SBM', 'FSAvg_FreeSurfer_SBM')),
))
async def test_sbmTransforms(workingDir: str,
                             modelLabel: str,
                             coordSysKeys: tuple[str, ...]):
    ...
    with tracer(workingDir, sessionKey, doOpen=False):
        ...
        # left and right hemisphere fsaverage seeds (approximately M1)
        lhSeed = np.array([[-37.0, -20.0, 60.0]], dtype=np.float64)
        rhSeed = np.array([[+37.0, -20.0, 60.0]], dtype=np.float64)

        for coordSysKey in coordSysKeys:
            assert coordSysKey in session.coordinateSystems, \
                f'Expected {coordSysKey} to be autogenerated for {modelLabel} session'
            coordSys = session.coordinateSystems[coordSysKey]

            # hemisphere correctness: laterality should survive atlas->native
            for seed in (lhSeed, rhSeed):
                native = coordSys.transformFromThisToWorld(seed)
                assert np.sign(native[0, 0]) == np.sign(seed[0, 0]), (
                    f'{coordSysKey}: atlas seed x={seed[0, 0]} mapped to native '
                    f'x={native[0, 0]} (hemisphere flipped)'
                )

            # round-trip atlas -> native -> atlas should stay close (SBM is
            # not bijective, so allow a few mm of resampling error but catch
            # gross hemisphere flips)
            seeds = np.vstack([lhSeed, rhSeed])
            native = coordSys.transformFromThisToWorld(seeds)
            atlasRoundtrip = coordSys.transformFromWorldToThis(native)
            maxAxisErr = float(np.max(np.abs(atlasRoundtrip - seeds)))
            logger.info(f'{coordSysKey}: atlas->native->atlas max axis error '
                        f'= {maxAxisErr:.2f} mm')
            assert maxAxisErr < 10.0, (
                f'{coordSysKey}: round-trip error {maxAxisErr:.2f} mm exceeds 10 mm'
            )
