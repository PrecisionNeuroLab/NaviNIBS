...
class Target(GenericCollectionDictItem[str]):
    ...
    _templateTargetCoord: np.ndarray | None = None
    """
    Can specify a target coordinate in template space (e.g. MNI space), and then the primary targetCoord can be 
     autogenerated from this. Note that this template coord is not updated during later target coordinate changes, so 
     may be out of date if the targetCoord is edited after initial creation.
    """
    _templateCoordinateSystemKey: str | None = None
    """
    The coordinate system in which templateTargetCoord is defined, e.g. 'MNI_SimNIBS12DoF'
    """

    _cachedCoilToMRITransf: np.ndarray | None = attrs.field(init=False, default=None)
    """
    If coilToMRITransf isn't manually set, it might be auto-calculated from other target parameters.
    """
    _cachedCoilAngle: float | None = attrs.field(init=False, default=None)
    """
    ...
    def targetCoord(self, newTargetCoord: np.ndarray | None):
    ...
    @entryCoord.setter
    @collectionDictItemAttrSetter(equalityFn=array_equalish, extraAttrsToSignalOnChange=['coilToMRITransf'])
    def entryCoord(self, newEntryCoord: np.ndarray | None):
        newCoilToMRITransf = calculateCoilToMRITransfFromTargetEntryAngle(
            session=self.session,
    ...
    @depthOffset.setter
    @collectionDictItemAttrSetter(extraAttrsToSignalOnChange=['coilToMRITransf'])
    def depthOffset(self, newDepthOffset: float | None):
        newCoilToMRITransf = calculateCoilToMRITransfFromTargetEntryAngle(
            session=self.session,
            targetCoord=self._targetCoord,
            entryCoord=self._entryCoord,
    ...
    @property
    def entryCoordPlusDepthOffset(self) -> np.ndarray | None:
    ...
    def coilToMRITransf(self, newCoilToMRITransf: np.ndarray | None):
        """
    ...
    def templateTargetCoord(self):
        return self._templateTargetCoord

    @templateTargetCoord.setter
    @collectionDictItemAttrSetter(equalityFn=array_equalish)
    def templateTargetCoord(self, newTemplateTargetCoord: np.ndarray| None):
        """
        Note: for now, must manually call autosetTargetCoordFromTemplate() to update targetCoord after changing this.
        """
        # TODO: maybe auto-update targetCoord and autoset entry if this and coordinateSystemKey are set to not None?
        pass

    @property
    def templateCoordinateSystemKey(self):
        return self._templateCoordinateSystemKey

    @templateCoordinateSystemKey.setter
    @collectionDictItemAttrSetter()
    def templateCoordinateSystemKey(self, newTemplateCoordinateSystemKey: str | None):
        """
        Note: for now, must manually call autosetTargetCoordFromTemplate() to update targetCoord after changing this.
        """
        # TODO: maybe auto-update targetCoord and autoset entry if this and coordinateSystemKey are set to not None?
    ...
    def session(self, session: Session):
        if self._session is not session:
            ...
            if prevSessionWasNone and self.targetCoord is None \
                    and (self.templateTargetCoord is not None and self.templateCoordinateSystemKey is not None):
                self.autosetTargetCoordFromTemplate()
    ...
    def autosetTargetCoordFromTemplate(self):
        if self.templateTargetCoord is None or self.templateCoordinateSystemKey is None:
            raise ValueError('Cannot autoset target coord from template: missing templateTargetCoord or templateCoordinateSystemKey')

        if self.session is None:
            raise ValueError('Cannot autoset target coord from template: missing session info')

        coordSys = self.session.coordinateSystems.get(self.templateCoordinateSystemKey, None)
        if coordSys is None:
            raise KeyError(f'Cannot autoset target coord from template: no coordinate system with key {self.templateCoordinateSystemKey} in session')

        self.targetCoord = coordSys.transformFromThisToWorld(self.templateTargetCoord)
        logger.info(f'Autoset target coord for target {self.key} from template coord sys {self.templateCoordinateSystemKey} to {self.targetCoord}')
    ...
    def asDict(self) -> tp.Dict[str, tp.Any]:
        return attrsWithNumpyAsDict(self,
                                    npFields=('targetCoord', 'entryCoord', 'coilToMRITransf', 'templateTargetCoord'),
                                    exclude=('session',))

    @classmethod
    def fromDict(cls, d: tp.Dict[str, tp.Any], session: Session | None = None):
        o = attrsWithNumpyFromDict(cls, d, npFields=('targetCoord', 'entryCoord', 'coilToMRITransf', 'templateTargetCoord'),)
        o.session = session

        # auto-set target coordinate based on template if targetCoord not specified and template coordinate provided
        if o.targetCoord is None and (o.templateTargetCoord is not None or o.templateCoordinateSystemKey is not None):
            if o.templateTargetCoord is None or o.templateCoordinateSystemKey is None:
                logger.warning(f'No template target coord or coordinate system key for target {o.key}, cannot autogenerate target coord')
            elif session is not None:
                o.autosetTargetCoordFromTemplate()
            else:
                logger.warning(f'No session info with which to autogenerate target coord for target {o.key} from template coord sys {o.templateCoordinateSystemKey}')
