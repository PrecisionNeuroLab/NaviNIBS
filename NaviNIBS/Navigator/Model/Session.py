...
class Session:
    ...
    def _onCoordinateSystemsChanged(self, keys: list[str], changedAttrs: tp.Optional[list[str]] = None):
        # Changes to autogenerated coord systems' internal state (transforms, file
        # paths) don't need to persist — they re-derive from the head model on load.
        # But user-tunable fields that round-trip via PlaceholderCoordinateSystem
        # (currently just isVisible) must mark the session dirty regardless.
        persistedAutogenAttrs = {'isVisible'}
        isDirty = False
        for key in keys:
            try:
                cs = self.coordinateSystems[key]
            except KeyError:
                # coordinate system was presumably deleted or renamed
                isDirty = True
                break
            if not cs.isAutogenerated:
                isDirty = True
                break
            if changedAttrs is None or any(a in persistedAutogenAttrs for a in changedAttrs):
                isDirty = True
                break
        if isDirty:
